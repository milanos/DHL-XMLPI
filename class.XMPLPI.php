<?php
class XMLPI{
   //private $mode='live'; // $mode = (ENUM('test','live'))
   //public $mode='test';
   // $mode = (ENUM('test','live'))
   private $server;
   private $credentials = 
					array(
							'SiteID' => array(
											'test' => 'v62_nwTCQbOW4z',		// test account
											'live'=>'v62_AlDSsPP7Vw'		// production account
											),
							'Password' => array(
											'test' => '3mJLpkuG9o',		// test account
											'live' => 'ABFb6YuS4y'		// production account
											)
						);
   private $xml_in;
   private $xml_out;
   ...
   public function XMLPI(){
	  ...
	  public function create_xml_out($data){
		  ...
		  elseif($this->mode=='live')
		  {
			...
			// this is a third-party bypass server
			//$this->server='https://app.dhlexpress.pl/keelog/keelog.php';
		  }else{
		...
		$xml=new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?'.'><DHLXML/>');
			$Request=$xml->addChild('Request');
				...
				$ServiceHeader->addChild("MessageReference",time()."_______".date("Ymdhis"));
				...
				$MetaData=$Request->addChild('MetaData');
					$MetaData->addChild('SoftwareName','Keelog');
					$MetaData->addChild('SoftwareVersion','1.1');
			$xml->addChild('LanguageCode','en');
			//$xml->addChild('PiecesEnabled','Y'); //mie wystepuje w Scherma 10
			...
			$Consignee=$xml->addChild('Consignee');
				...
				//$Commodity->addChild('CommodityCode','1');
				//$Commodity->addChild('CommodityName','String');
		...
		//file_put_contents('xml_request.xml',$this->xml_out);
		//$this->tab('Plik zapisany');
	  }
   ...
   public static function tab($tab,$tekst=''){
	  ...
	  private function post_url($url, $data,$timeout=200) {
			...
			// display the results
			//$this->tab($xmlResponse,'$xmlResponse');
